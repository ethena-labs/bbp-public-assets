...
contract StakedUSDeTest is Test, IERC20Events {
  ...
  function testDonationAttack() public {
    ...
    uint256 bobsUSDeBal = stakedUSDe.balanceOf(bob);
    uint256 bobsUSDeExpectedBal = (bobStake * initialStake) / (initialStake + donationAmount);
    assertApproxEqAbs(bobsUSDeBal, bobsUSDeExpectedBal, 1e9);
    assertTrue(bobsUSDeBal > 0);
  }
}
