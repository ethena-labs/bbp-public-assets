...
contract StakedUSDeBlacklistTest is Test, IERC20Events {
  ...
  function testCanBurnOnRedistribute() public {
    ...
    uint256 aliceStakedBalance = stakedUSDe.balanceOf(alice);
    uint256 previousTotalSupply = stakedUSDe.totalSupply();
    uint256 previousUSDeBalance = usdeToken.balanceOf(address(stakedUSDe));
    uint256 previousUSDePerSUSDe = stakedUSDe.totalAssets() / stakedUSDe.totalSupply();
    assertEq(aliceStakedBalance, _amount);
    ...
    assertEq(usdeToken.balanceOf(address(stakedUSDe)), previousUSDeBalance);
    assertEq(stakedUSDe.totalAssets() / stakedUSDe.totalSupply(), previousUSDePerSUSDe);
    assertTrue(usdeToken.balanceOf(address(stakedUSDe)) > stakedUSDe.totalAssets());
    vm.warp(block.timestamp + 8 hours);
    assertEq(usdeToken.balanceOf(address(stakedUSDe)), stakedUSDe.totalAssets());
  }
  ...
  function testCantBurnOnRedistributeWhileVesting() public {
    ...
    stakedUSDe.redistributeLockedAmount(alice, address(0));
    vm.stopPrank();
    assertEq(stakedUSDe.balanceOf(alice), _amount);
  }
