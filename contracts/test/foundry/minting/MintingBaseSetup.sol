...
contract MintingBaseSetup is Test, IEthenaMintingEvents, IUSDeDefinitions {
  ...
  function setUp() public virtual {
    ...
    EthenaMintingContract = new EthenaMinting(
      IUSDe(address(usdeToken)), weth, assets, custodians, owner, _maxMintPerBlock, _maxRedeemPerBlock
    );
    ...
    EthenaMintingContract.grantRole(collateralManagerRole, collateralManager);
    ...
    )
  {
    ...
    collateralToken.approve(address(EthenaMintingContract), collateralAmount);
    vm.stopPrank();

    if (!multipleMints) {
      assertEq(usdeToken.balanceOf(beneficiary), 0, "Mismatch in USDe balance");
      assertEq(collateralToken.balanceOf(address(EthenaMintingContract)), 0, "Mismatch in collateral balance");
  ...
  function redeem_setup(
    ...
    uint256 nonce,
    bool multipleRedeem
  ) public returns (IEthenaMinting.Order memory redeemOrder, IEthenaMinting.Signature memory takerSignature2) {
    (
      IEthenaMinting.Order memory mintOrder,
      IEthenaMinting.Signature memory takerSignature,
      IEthenaMinting.Route memory route
    ) = mint_setup(usdeAmount, collateralAmount, collateralAsset, nonce, false);
    ...
    if (!multipleRedeem) {
      assertEq(
        collateralAsset.balanceOf(address(EthenaMintingContract)), collateralAmount, "Mismatch in collateral balance"
      );
